...
function proveraPodataka () {
	...
	var emailRi = /^(([a-z\d]+\.{1}){2}\d{1,3}\.\d{2}@ict.edu.rs)|(([a-z\d]+\.*)+@(gmail|hotmail|yahoo)\.com)$/;
	var passRi = /^[\w\d\S]{8,25}$/

	if(!imeRi.test(ime.value)){
		document.querySelector("#cf-fname").style.borderBottomColor = '#ce3232';
		return false	
	}
	else{
	...
	if(!prezimeRi.test(prezime.value)){
		document.querySelector("#cf-lname").style.borderBottomColor = '#ce3232';
		return false	
	}
	else{
	...
	// 	document.querySelector("#cf-userName").style.borderBottomColor = '#ce3232';
	// 	return false	

	// }
	// else{
	// 	document.querySelector("#cf-userName").style.borderBottomColor = '#ddd';
	// }

	if(!emailRi.test(email.value)){
		document.querySelector("#cf-email").style.borderBottomColor = '#ce3232';
		return false	
	...
	if(!emailRi.test(rEmail.value)){
		document.querySelector("#cf-repEmail").style.borderBottomColor = '#ce3232';
		return false	
	}
	else{
		document.querySelector("#cf-repEmail").style.borderBottomColor = '#ddd';
	}
