...
function prikazProivoda(){
	...
	// --------------------RADJENO SA PROIZVODIMA SAMO KOJI ZADOVOLJAVAJU IZ BAZE--------------------------------------------
		...
		return `
			<tr class="rem1">
					<td class="invert-image">
						<a href="single.html">
							<img src="${pro.slika}" alt=" " class="img-responsive">
						</a>
					</td>
					<td class="invert">${pro.naziv}</td>
					<td class="invert">${pro.cena}</td>
					<td class="invert">${pro.kolicina}</td>
					<td class="invert">${pro.cena * pro.kolicina} RSD</td>
					<td class="invert">
						<div class="rem">
							<div class=""><button onclick='izbrisiIzKorpe(${pro.id})'>Remove</button> </div>
						</div>
					</td>
				</tr>`
	}
